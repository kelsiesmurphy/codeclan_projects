var movies = [
  ...
  for (var movie of movies) {
    if (movie.title.toLowerCase() == 'citizen kane') {
        console.log(`Citizen Kane year of release is ${movie.year}`)
    }
  }
  
  // 3. Using a different kind of loop, iterate through the movies and log each movie's title and audience rating.
  for (var i = 0; i < movies.length; i++) {
    console.log(`The movie title is: ${movies[i].title.toLocaleLowerCase}, the audience rating is ${movies[i].ratings.audience}`)
  }